    ...
    PriceCalculator
{
     ...
     */
    function payFees(uint256 _nftId, uint256 _tier) public nonReentrant {
        ...
        uint256 stakingRewardsAvailable = 0;
        ...
        if (stakingRewardsAvailable >= ovrToPay) {
            ...
            if (stakingRewardsAvailable > 0) {
                ...
                require(
                    OVR.balanceOf(_msgSender()) >= remainingTokens,
                    "Not enough OVR"
                );
                require(
                    OVR.transferFrom(_msgSender(), treasury, remainingTokens),
                    "Insufficent allowance"
                );
            } else {
                require(
                    OVR.balanceOf(_msgSender()) >= ovrToPay,
                    "Not enough OVR"
                );
                require(
                    OVR.transferFrom(_msgSender(), treasury, ovrToPay),
                    "Insufficent allowance"
                );
            }
        }

        if (_msgSender() == land.ownerOf(_nftId)) {
            landRenting.activateNoRentFromHosting(
                _nftId,
                tiers[_tier].duration
            );
        }

        payments[_nftId] = Payment(
            tiers[_tier].weight,
            tiers[_tier].duration,
            ovrToPay,
            _now()
