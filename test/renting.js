...
describe("OVRLand Renting - TEST", async () => {
  let Renting, renting;
  let ProxyRenting;
  let OVRLand, ovrLand;
  ...
  describe("First Round", async () => {
    ...
    it("Owner should accept offer", async () => {
      ...
      await ProxyRenting.connect(addr2).acceptOffer(2);
    ...
    it("Owner should activate no rent", async () => {
      ...
      await ProxyRenting.activateNoRent(1);
    ...
    it("Renter should accept offer, and Lands should be rented", async () => {
      ...
      await ProxyRenting.connect(addr6).acceptOffer(3);
      ...
      const rented = await ovrLandExperience.isOVRLandRented(3);
  ...
  describe("Second Round", async () => {
    ...
    it("Should FAIL: Renter(Addr7) try to accept offer(50 OVR, 1 month) of LandId 4 owned by Addr4 before 24h wait", async () => {
      ...
      await expect(
        ProxyRenting.connect(addr7).acceptOffer(4)
      ).to.be.revertedWith("24 hours not yet elapsed");
    });
    ...
    it("Should FAIL: Real Offerer try to accept 9 days after after", async () => {
      ...
      await expect(
        ProxyRenting.connect(addr8).acceptOffer(4)
    ...
    it("pay hosting fees", async () => {
      ...
      await hosting.payFees(1, 0);
    });
    it("noRent is active?", async () => {
      const active = await ProxyRenting.isNoRentActive(1);
